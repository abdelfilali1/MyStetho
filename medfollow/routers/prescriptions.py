...
async def list_prescriptions(request: Request, db: aiosqlite.Connection = Depends(get_db)):
    user = get_current_user(request)
    if not user:
        return RedirectResponse(url="/login", status_code=302)

    cursor = await db.execute(
        """SELECT pr.*, p.first_name || ' ' || p.last_name AS patient_name, u.first_name || ' ' || u.last_name AS doctor_name, COUNT(pi.id) AS item_count FROM prescriptions pr JOIN patients p ON pr.patient_id = p.id JOIN users u ON pr.doctor_id = u.id LEFT JOIN prescription_items pi ON pr.id = pi.prescription_id GROUP BY pr.id ORDER BY pr.prescription_date DESC LIMIT 50"""
    )
    prescriptions = [dict(r) for r in await cursor.fetchall()]

    return templates.TemplateResponse(
        "prescriptions/list.html",
        {"request": request, "user": user, "active": "prescriptions", "prescriptions": prescriptions},
    )
